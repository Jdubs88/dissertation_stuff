...
likelihood <- function(fc, b) {
  ...
  sum1 <- exp(v433361110/lambda1) + 
		      exp(v35318559 /lambda1) + 
		      exp(v433371133/lambda1)
		  
  sum2 <- exp(v648672039/lambda2) + 
		      exp(v268272787/lambda2) + 
		      exp(v124081067/lambda2) 
		  
  sum3 <- exp(v433381135/lambda3) + 
		      exp(v373484017/lambda3) + 
		      exp(v43028216 /lambda3) + 
		      exp(v112971485/lambda3) 
		  
  sum4 <- exp(v11788689 /lambda4) + 
		      exp(v359182079/lambda4) + 
		      exp(v35317555 /lambda4) + 
		      exp(v256084476/lambda4) 
		  
  sum5 <- exp(v363052474/lambda5) + 
		      exp(v363082477/lambda5) + 
		      exp(v11777685 /lambda5) 
  
  choicesum1 <- choice_data$chosen_648672039 * exp(v648672039/lambda1) + 
				        choice_data$chosen_268272787 * exp(v268272787/lambda1) + 
				        choice_data$chosen_124081067 * exp(v124081067/lambda1)
               
  choicesum2 <- choice_data$chosen_648672039 * exp(v648672039/lambda2) + 
				        choice_data$chosen_268272787 * exp(v268272787/lambda2) + 
				        choice_data$chosen_124081067 * exp(v124081067/lambda2)
               
  choicesum3 <- choice_data$chosen_433381135 * exp(v433381135/lambda3) + 
				        choice_data$chosen_373484017 * exp(v373484017/lambda3) + 
				        choice_data$chosen_43028216  * exp(v43028216 /lambda3) + 
				        choice_data$chosen_112971485 * exp(v112971485/lambda3)
               
  choicesum4 <- choice_data$chosen_11788689  * exp(v11788689 /lambda4) + 
				        choice_data$chosen_359182079 * exp(v359182079/lambda4) + 
				        choice_data$chosen_35317555  * exp(v35317555 /lambda4) + 
				        choice_data$chosen_256084476 * exp(v256084476/lambda4) 
               
  choicesum5 <- choice_data$chosen_363052474 * exp(v363052474/lambda5) + 
				        choice_data$chosen_363082477 * exp(v363082477/lambda5) + 
				        choice_data$chosen_11777685  * exp(v11777685 /lambda5) 
		  ...
		  prob_nest <-  
                (exp(lambda1 * logsum1) *  (choice_data$chosen_433361110) + 
											                      choice_data$chosen_35318559 ) + 
											                      choice_data$chosen_433371133)) + 
                  exp(lambda2 * logsum2) * (choice_data$chosen_648672039) + 
											                      choice_data$chosen_268272787) + 
											                      choice_data$chosen_124081067)) + 
                  exp(lambda3 * logsum3) * (choice_data$chosen_433381135) + 
											                      choice_data$chosen_373484017) + 
											                      choice_data$chosen_43028216 ) + 
											                      choice_data$chosen_112971485)) + 
                  exp(lambda4 * logsum4) * (choice_data$chosen_11788689 ) 
											                      choice_data$chosen_359182079) 
											                      choice_data$chosen_35317555 ) 
											                      choice_data$chosen_256084476)) + 
                  exp(lambda5 * logsum5) * (choice_data$chosen_363052474) + 
											                      choice_data$chosen_363082477) 
											                      choice_data$chosen_11777685 ))/ 
				                                   (exp(lambda1  * logsum1 ) +
				                                    exp(lambda2  * logsum2 ) +
				                                    exp(lambda3  * logsum3 ) +
				                                    exp(lambda4  * logsum4 ) +
...
gINFOSKIP <- 50         # How frequently to print info about the iteration process
modelname = "test1"
control <- list(
  modelname = modelname,
